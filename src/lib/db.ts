...
export function updateMember(id: string, updates: Partial<Omit<Member, 'id' | 'created_at'>>): { error: string | null } {
  const members = getMembers();
  ...
  if (updates.registration_number && members.some(m => m.registration_number === updates.registration_number && m.id !== id && m.active)) {
    return { error: 'Matrícula já cadastrada.' };
  }
